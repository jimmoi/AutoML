...
def create_pipeline(num_cols, cat_cols, args, limit_n_feature=10):
    ...
    def top_k_generate(n_column, n_choice, power=3):
        func2 = lambda x: np.ceil(n_column*(1e3**((x/x.max())-1)))
        test = np.arange(1,n_choice+1)
        test[1:] = func2(test)[:-1]
        return set(test.tolist())
    TOP_K = top_k_generate(feature_amount, limit_n_feature, 2)
    for k in TOP_K:
...
def main(args):
    ...
    optimizer = ACOOptimizer(dag, n_ants=20, iterations=10, local_search_iters=5, timeout=300)
